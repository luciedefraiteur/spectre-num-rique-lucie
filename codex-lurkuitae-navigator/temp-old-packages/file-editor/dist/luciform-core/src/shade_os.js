...
exports.invokeShadeOs = invokeShadeOs;
const base_1 = require("./types/base");
const llm_interface_1 = require("./llm_interface");
const osHint_1 = require("./utils/osHint");
 ...
 */
async function invokeShadeOs(command, invoker, previousRitual, previousError, lucie_s_voice) {
    console.log(`shadeOs received command: "${command}" from ${invoker} on OS: ${osHint_1.osHint}`);
    let prompt = `
        ...
        The target operating system is: ${osHint_1.osHint}.
        The detected shell is: ${osHint_1.detectedShell || 'unix'}.
    `;
    ...
    try {
        ...
        if (invoker === 'chad_orveil') {
            ...
            return await llm_interface_1.LLMInterface.query(chadPrompt, base_1.LLMModel.Gemini);
        }
        const llmResponse = await llm_interface_1.LLMInterface.query(prompt, base_1.LLMModel.Mistral);
        console.log(`[shadeOs] LLM Response: ${llmResponse}`);
        await (0, log_writers_1.logPersonaAction)(invoker, command, llmResponse);
        return llmResponse;
    }
    catch (error) {
...
//# sourceMappingURL=shade_os.js.map