...
export interface RitualExecutionStatus
{
    ...
    failedStep?: number;
    error?: string;
    movementFrame?: MovementFrame; // New: Optional movement frame data
}
...
export interface MovementFrame {
  currentStep: number;
  history: { step: number; outcome: string; }[];
...
export interface RitualContext
{
  ...
  // New: File Context and Pointer
  currentFile?: string;
  fileContent?: string;
  filePointer?: number; // Character index within fileContent
  textBeforePointer?: string; // Content before the pointer
  textAfterPointer?: string;  // Content after the pointer
  currentDirectory?: string; // Explicitly track current directory
}
...
export type ActionNode = PromenadeActionNode | JsonActionNode | MessageActionNode | AskPersonaActionNode | EntangleWithActionNode | ScryOrbActionNode | HelpRequestActionNode | SearchAndReplace | Insert | Delete | Append | ShellCommand | CreateFile | LlmOperation | JsonData | ReadLines | ArcaneInstruction | Prompt | Glob | WebFetch | Test | Output | ErrorOperation | Variable | Update | Help | Debug | Yaml | Key | Query | Batch | Note | Raw | Code | Data | FileOperation | Git | Hash | Info | Json | Log | Message | Name | Option | Path | Result | Status | Text | Url | Value | Warning | Xml | Yes | Zip | ExecuteTypescriptFile | RitualModificationInstruction | ScryOrb | ModulateMemory | EntangleWith | AskLucie | ReflectiveLoop | TransmuteFile | ApplyTransmutation | NavigateFile | ModifyFileAtPointer;
