...
export class Base666Encoder {
     ...
     */
    static calculateBlasphemy(char, charCode, position, fullText) {
        let blasphemy = 0;
        ...
        if (char.match(/[⛧𖤐𝖚⟁⇌↯⟲ⱷ𓂀𓆩⫷]/))
            blasphemy += 200;
        if (char.match(/[𝖋𝖆𝖎𝖗𝖊𝖚𝖗𝖆𝖗𝖈𝖍𝖎𝖙𝖊𝖈𝖙𝖊]/))
            blasphemy += 300;
     ...
     */
    static analyzeContext(char, position, text) {
     ...
     */
    static reverseBlasphemy(hellChar, hellIndex) {
        // Mapping inverse approximatif
        if (hellIndex < 40)
            return String.fromCharCode(97 + (hellIndex % 26)); // a-z
        if (hellIndex < 80)
            return String.fromCharCode(65 + (hellIndex % 26)); // A-Z
        if (hellIndex < 120)
            return String.fromCharCode(48 + (hellIndex % 10)); // 0-9
        if (hellIndex < 200)
            return ['!', '@', '#', '$', '%', '^', '&', '*'][hellIndex % 8];
    ...
    static encodeSignature(name) {
        ...
        let signature = '⛧';
        if (blasphemyLevel > 300)
            signature += '𖤐';
        if (blasphemyLevel > 500)
            signature += '𓂀';
        signature += encoded;
        if (blasphemyLevel > 500)
            signature += '𓆩';
     ...
     */
    static calculateTotalBlasphemy(text) {
        let total = 0;
        for (let i = 0; i < text.length; i++) {
     ...
     */
    static generateInjectionSignature(creators) {
