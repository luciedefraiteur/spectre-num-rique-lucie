...
async function animateCommand(args) {
    const luciformPath = args[0];
    const animationType = args[1] || AnimationType.FULL_ANIMATION;
    if (!luciformPath) {
...
async function dialogueCommand(args) {
    const luciformPath = args[0];
...
async function scryorbCommand(args) {
    const luciformPath = args[0];
    if (!luciformPath) {
