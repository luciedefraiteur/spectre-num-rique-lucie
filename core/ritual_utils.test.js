...
async function testReplanLogic() {
    ...
    const mockHandleAnalyse = async (incantation, context, index, plan) => {
        analysisCounter++;
    ...
    const mockHandleCommande = async (incantation, context, plan) => {
    ...
    const mockGenerateRitual = async (input, context, model, analysisResult) => {
        const newPlanAfterAnalysis = {
    ...
    await executeRitualPlan(planToExecute, context, ask, {
        ...
        stepHandlers: mockStepHandlers,
    });
    // --- Assertions ---
    console.log(colorize("[TEST] Verifying assertions...", Colors.FgCyan));
    const executedStepTypes = planToExecute.incantations.map((e) => e.type);
...
Promise.race([
    testReplanLogic(),
    new Promise((_, reject) => setTimeout(() => reject(new Error(`Test timed out after ${testTimeout}ms`)), testTimeout))
]).catch(err => {
