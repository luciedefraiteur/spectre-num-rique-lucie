...
async function globalSetup() {
    ...
    LLMInterface.query = async (prompt) => {
...
tests.push(async () => {
    await runTest("generateAndSaveMemoryFragment should create a new leaf with poetic summary", async () => {
        ...
        await generateAndSaveMemoryFragment(context, lastResult, { title: 'test plan', goal: 'test goal', incantations: [], complexity: 'simple', sequence: 0 }, 0, branchPath, TEST_MEMORY_ROOT);
...
tests.push(async () => {
    await runTest("appendToVector should append an entry to vector_of_intent.log", async () => {
        ...
        await appendToVector(context, TEST_MEMORY_ROOT);
...
tests.push(async () => {
    await runTest("updateConstellationMap should update constellation_map.json", async () => {
        ...
        await updateConstellationMap(context, TEST_MEMORY_ROOT);
...
executeAllTests();
//# sourceMappingURL=memory_weaver.test.js.map