    ...
    public generate(node: ASTNode | ASTNode[]): string
    {
        ...
        switch(node.constructor)
        {
            ...
            case FunctionDeclarationNode:
                ...
                const params = funcDecl.params.map((p: ASTNode) => this.generate(p)).join(', ');
                const body = funcDecl.body.map((b: ASTNode) => this.indent() + this.generate(b)).join('\n');
                this.indentLevel++;
                const indentedBody = funcDecl.body.map((b: ASTNode) => this.generate(b)).join('\n');
                this.indentLevel--;
...
${ this.indent() }}`;
            case IfStatementNode:
                ...
                this.indentLevel++;
                const thenBranch = ifStmt.thenBranch.map((s: ASTNode) => this.generate(s)).join('\n');
                this.indentLevel--;
                let elseBranch = '';
                if(ifStmt.elseBranch)
                {
                    this.indentLevel++;
                    elseBranch = ` else {
${ ifStmt.elseBranch.map((s: ASTNode) => this.generate(s)).join('\n') }
${ this.indent() }}`;
                    this.indentLevel--;
                }
...
${ this.indent() }}${ elseBranch }`;
            ...
            case WhileStatementNode:
                ...
                this.indentLevel++;
                const whileBody = whileStmt.body.map((s: ASTNode) => this.generate(s)).join('\n');
                this.indentLevel--;
...
${ this.indent() }}`;
            case ForStatementNode:
                ...
                const forBody = forStmt.body.map((s: ASTNode) => this.generate(s)).join('\n');
                this.indentLevel--;
...
${ this.indent() }}`;
            ...
            case CallExpressionNode:
                ...
                const args = callExpr.args.map((arg: ASTNode) => this.generate(arg)).join(', ');
                return `${ callee }(${ args })`;
            ...
            case ImportDeclarationNode:
                ...
                {
                    const imports = importDecl.imports.map((imp: IdentifierNode | {alias: IdentifierNode; name: IdentifierNode;}) =>
                    {
                        if('alias' in imp)
                        {
