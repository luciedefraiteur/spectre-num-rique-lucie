...
exports.initializePersonas = initializePersonas;
exports.getPersona = getPersona;
exports.parseLuciformDocument = parseLuciformDocument;
const tokenizer_js_1 = require("./tokenizer.js");
const persona_loader_js_1 = require("../persona_loader.js");
let loadedPersonas = [];
function initializePersonas() {
    loadedPersonas = (0, persona_loader_js_1.loadPersonas)('../personas/src');
}
...
function parseLuciformDocument(luciformContent, logRitual, logFileName) {
    ...
    const tokenizer = new tokenizer_js_1.LuciformTokenizer(luciformContent);
    ...
    const skipNewlines = () => {
        while (peek().type === tokenizer_js_1.LuciformTokenType.NEWLINE) {
            consume(tokenizer_js_1.LuciformTokenType.NEWLINE);
        }
    };
    let sygil;
    if (peek().type === tokenizer_js_1.LuciformTokenType.LUCIFORM_SYGIL) {
        sygil = consume(tokenizer_js_1.LuciformTokenType.LUCIFORM_SYGIL).value;
        skipNewlines();
    }
    ...
    const parsePas = () => {
        ...
        skipNewlines();
        while (peek().type !== tokenizer_js_1.LuciformTokenType.PAS_SEPARATOR && peek().type !== tokenizer_js_1.LuciformTokenType.EOF) {
            const token = peek();
            if (token.type === tokenizer_js_1.LuciformTokenType.ACTION_START) {
                consume(tokenizer_js_1.LuciformTokenType.ACTION_START);
                let actionBlockContent = '';
                // Consume content until the next PAS_SEPARATOR or EOF
                while (peek().type !== tokenizer_js_1.LuciformTokenType.PAS_SEPARATOR && peek().type !== tokenizer_js_1.LuciformTokenType.EOF) {
                    const actionToken = peek();
                    if (actionToken.type === tokenizer_js_1.LuciformTokenType.NEWLINE) {
                        consume(tokenizer_js_1.LuciformTokenType.NEWLINE);
                        actionBlockContent += '\n';
                    }
                    else {
            ...
            else if (token.type === tokenizer_js_1.LuciformTokenType.LEGACY_COMMAND) {
                actionNode = parseLegacyCommand(consume(tokenizer_js_1.LuciformTokenType.LEGACY_COMMAND).value, logRitual, logFileName);
    ...
    if (peek().type === tokenizer_js_1.LuciformTokenType.PAS_SEPARATOR) {
        consume(tokenizer_js_1.LuciformTokenType.PAS_SEPARATOR);
        skipNewlines(); // Skip newlines after the initial PAS_SEPARATOR
    }
    while (peek().type !== tokenizer_js_1.LuciformTokenType.EOF) {
