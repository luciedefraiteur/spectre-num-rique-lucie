...
const generateWaitMessagePrompt_js_1 = require("./prompts/generateWaitMessagePrompt.js");
...
class LLMInterface {
    static async query(prompt, model = base_1.LLMModel.Mistral, _fetch = fetch) {
        const cacheKey = `${model}-${prompt}`;
        if (LLMInterface.cache.has(cacheKey)) {
            return LLMInterface.cache.get(cacheKey);
        }
        let actualModel = model;
        if (model === base_1.LLMModel.Random) {
            let availableModels = Object.values(base_1.LLMModel).filter(m => m !== base_1.LLMModel.Random);
            const hasOpenAI = process.env.OPENAI_API_KEY;
            if (!hasOpenAI) {
                availableModels = availableModels.filter(m => m !== base_1.LLMModel.OpenAI);
            }
            actualModel = availableModels[Math.floor(Math.random() * availableModels.length)];
        }
        if (actualModel === base_1.LLMModel.OpenAI) {
        ...
        else if (actualModel === base_1.LLMModel.Gemini) {
        ...
        else if (actualModel === base_1.LLMModel.Claude) {
        ...
        else if (actualModel === base_1.LLMModel.CodeLlama || actualModel === base_1.LLMModel.CodeLlamaCode || actualModel === base_1.LLMModel.Llama3 || actualModel === base_1.LLMModel.Mistral) {
