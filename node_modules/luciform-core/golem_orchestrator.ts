...
export async function orchestrateGolem(luciformPath: string) {
    ...
    let luciformDocument: LuciformDocument; // Declare luciformDocument here
    try {
        ...
        const transmutationRitual: RitualPlan = {
    ...
    const reconstructionAction: JsonActionNode = {
      type: "json_action",
      operation: {
        type: "shell_command",
    ...
    const isLastStepReconstruction = lastPas && lastPas.action &&
                                     ...
                                     (lastPas.action.operation as any).type === "shell_command" &&
                                     (lastPas.action.operation as any).command === reconstructionAction.operation.command;
    ...
    if (luciformDocument.pas.length > 0) {
        ...
        if (firstStep.action && firstStep.action.type === 'json_action') {
            const operation = firstStep.action.operation as any;
            if (operation.type === 'spawn_golem' && operation.golemConfig) {
                ...
                const remainingRitual: RitualPlan = {
