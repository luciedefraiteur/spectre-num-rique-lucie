...
var ImportDeclarationNode = /** @class */ (function (_super) {
    __extends(ImportDeclarationNode, _super);
    function ImportDeclarationNode(imports, moduleSpecifier) {
        var _this = _super.call(this) || this;
        _this.imports = imports;
        _this.moduleSpecifier = moduleSpecifier;
        return _this;
    }
...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.importDeclaration = function () {
        ...
        if (this.match(lexer_js_1.TokenType.Punctuation, '{')) {
            // Named imports
            do {
                var name_1 = this.consume(lexer_js_1.TokenType.Identifier, 'Expect identifier in import.');
                var alias = void 0;
                if (this.match(lexer_js_1.TokenType.Keyword, 'as')) {
                    alias = new IdentifierNode(this.consume(lexer_js_1.TokenType.Identifier, 'Expect alias after \'as\'.').text);
                }
                imports.push(alias ? { name: new IdentifierNode(name_1.text), alias: alias } : new IdentifierNode(name_1.text));
        ...
        var moduleSpecifier = new StringLiteralNode(this.consume(lexer_js_1.TokenType.StringLiteral, 'Expect module specifier.').text);
    ...
    Parser.prototype.expression = function () {
        ...
        while (true) {
            if (this.match(lexer_js_1.TokenType.Punctuation, '(')) {
                ...
                var name_2 = this.consume(lexer_js_1.TokenType.Identifier, 'Expect property name after \'.\'.');
                expr = new PropertyAccessNode(expr, new IdentifierNode(name_2.text));
            }
            else if (this.match(lexer_js_1.TokenType.Operator)) {
    ...
    Parser.prototype.forStatement = function () {
        ...
        this.consume(lexer_js_1.TokenType.Punctuation, ';', 'Expect \';\' after loop initializer.');
        var condition;
        if (!this.check(lexer_js_1.TokenType.Punctuation, ';')) {
    ...
    Parser.prototype.callExpression = function (callee) {
        var args = [];
        this.consume(lexer_js_1.TokenType.Punctuation, '(', 'Expect \'(\' after function name.');
        if (!this.check(lexer_js_1.TokenType.Punctuation, ')')) {
            do {
