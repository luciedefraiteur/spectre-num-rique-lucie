...
class Context {
  private RX_DECIMAL: RegExp = /^-?(\.[0-9]+|[0-9]+(\.[0-9]+)?)([eE]-?[0-9]+)?$/;
  private RX_HEXA: RegExp = /^-?0x[0-9a-f]+$/i;
  private RX_OCTAL: RegExp = /^-?0o[0-7]+$/i;
  private RX_BINARY: RegExp = /^-?0b[01]+$/i;
  private eaters: Function[];
  ...
  private is(...args: string[]): boolean {
    let arg: string;
  ...
  private eos(): boolean { return this.index >= this.end; }
  ...
  private addToken(type: TokenType, index?: number, value?: any): void {
  ...
  private eatIdentifier(): void {
    ...
    if (this.RX_DECIMAL.test(str)) {
      ...
      let type: TokenType = TokenType.SPECIAL;
      let value: any = str;
      if (str === 'null')
        value = null;
