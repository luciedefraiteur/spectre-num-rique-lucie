...
async function modifyFile(filePath, startLine, endLine, newContent) {
    try {
        const content = await fs.readFile(filePath, 'utf-8');
        const lines = content.split('\n');

        if (startLine < 1 || startLine > lines.length || endLine > lines.length || startLine > endLine) {
            console.error(`Invalid line numbers provided. File has ${lines.length} lines. Start: ${startLine}, End: ${endLine}`);
            process.exit(1);
        }
...
if (args.length !== 4) {
    console.log('Usage: node file_modifier.js <filePath> <startLine> <endLine> <newContent>');
    process.exit(1);
}
